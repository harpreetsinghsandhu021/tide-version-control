...
class Repository
  class Migration 
    ...
    attr_reader :errors, :changes, :mkdirs, :rmdirs
    
    def initialize(repository, tree_diff)
      @repo = repository
      @diff = tree_diff

      # Holds Lists of files that need either creating, updating, or deleting
      @changes = { :create => [], :update => [], :delete => [] }
      @mkdirs = Set.new # collects directories we need to create
      @rmdirs = Set.new # collects directories we need to remove
    ...
    # High-level plan for the phases of migration
    def apply_changes
      plan_changes
      update_workspace
      update_index
    ...
    def index_differs_from_trees(entry, old_item, new_item)
      @inspector.compare_tree_to_index(old_item, entry) && @inspector.compare_tree_to_index(new_item, entry)
    end
    ...
    def collect_errors
      ...
      raise Conflict if !@errors.empty?
    end
